...
| `device_status_changed` (v2) | `IGameInput::RegisterDeviceCallback` (`GameInputDeviceAnyStatus`) | Emitted for status transitions other than connect/disconnect. Flags that arrive with the connect are already in `get_status()` when `device_connected` fires and emit nothing. |
